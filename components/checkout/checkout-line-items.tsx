...
export function CheckoutLineItems({ allowQuantityChange = true, handleQuantityChange, checkoutData, quantity }: Props) {
  return (
    <>
      <div className={'md:pt-12 text-base leading-[20px] font-medium'}>{checkoutData?.items[0].price_name}</div>
      {allowQuantityChange ? <QuantityField quantity={quantity} handleQuantityChange={handleQuantityChange} /> : null}
